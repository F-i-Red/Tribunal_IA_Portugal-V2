"""
Pipeline principal — usa a instância judicial para adaptar linguagem e prompts.
A ata é montada localmente (sem API) para robustez com modelos free e pagos.
"""
...
Importâncias: critica, relevante, complementar"""
    ...
    def process(self, case_description: str,
        ...
        def _chamar(nome: str, fn, *args) -> str:
            """Chama agente com fallback seguro para respostas None/vazias."""
        ...
        # Agentes — protegidos contra None e excepções
        detetive    = _chamar("detetive",         self._detetive,  anon_text, ctx_instrucao, inst)
        acusacao    = _chamar("acusacao",          self._acusacao,  anon_text, detetive, inst)
        defesa      = _chamar("defesa",            self._defesa,    anon_text, detetive, acusacao, inst)
        s_rigorosa  = _chamar("juiz_rigoroso",     self._juiz,      anon_text, detetive, acusacao, defesa, "rigoroso",    inst)
        s_garantista= _chamar("juiz_garantista",   self._juiz,      anon_text, detetive, acusacao, defesa, "garantista",  inst)
        s_equilibrada=_chamar("juiz_equilibrado",  self._juiz,      anon_text, detetive, acusacao, defesa, "equilibrado", inst)

        # Montar ata localmente
    ...
    # ─────────────────────────────────────────────────────────────
    # AGENTES — prompts adaptados por instância
    # ─────────────────────────────────────────────────────────────

    def _detetive(self, case_text: str, ctx_instrucao: str, inst: InstanciaJudicial) -> str:
...
Usa linguagem técnica portuguesa. Sê preciso e factual."""
    ...
    def _acusacao(self, case_text: str, detetive: str, inst: InstanciaJudicial) -> str:
...
Português europeu, linguagem jurídica formal."""
    ...
    def _juiz(self, case_text: str, detetive: str, acusacao: str,
              defesa: str, perfil: str, inst: InstanciaJudicial) -> str:
        ...
        # Prompt MUITO directivo para funcionar com modelos fracos
...
Base de conhecimento RAG: Código Penal, CPP, Código Civil, Código do Trabalho,
...
"""
    ...
    def _validar_e_adicionar_citacoes(self, acusacao: str, defesa: str,
                                         ...
                                         equilibrada: str) -> str:
        """Valida citações em todas as peças e gera relatório."""
    ...
    def _comparar(self, rigorosa: str, garantista: str, equilibrada: str,
        ...
        def dispositivo(txt: str) -> str:
            # Tenta extrair secção DISPOSITIVO
            m = re.search(
            ...
            # Fallback: procura verbo decisório
            m = re.search(
            ...
            # Último fallback: primeiras 200 chars da sentença
